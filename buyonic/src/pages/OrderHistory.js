...
export const OrderHistory = () => {
  ...
  const getProducts = () => {
    let config = {
      method: 'get',
      url: 'https://buyonic.herokuapp.com/product/order',
      headers: { 
  ...
  return (
    <div className="bg-gray-800 min-h-screen">
      ...
      <div className='px-48 py-12'>
        <h1 className='text-gray-200 text-5xl font-semibold pb-8'>Order History</h1>
        <div className='grid grid-cols-3 justify-center gap-16'>
          {productList && productList.map(product => (
            <div className='col-span-1 shadow-lg'>
              <OrderHistoryProduct product={product} key={product.id} />
            </div>
          ))}
        </div>
      </div>
